...
| W2.6 | ADR-006 FHIR R4 + Thai coding canonical | 0.5d | 🟡 | 2026-05-18 [ADR-006](decisions/ADR-006-fhir-canonical-design.md). Locks 5 W2.4 open questions: (1) Bundle.entry = closed enum with `#[serde(tag="resourceType")]`; (2) Versioning = audit-via-Tyr (no in-resource meta storage); (3) JSON Schema via `schemars` derive + manual FHIR cardinality annotations; (4) Parsing = hybrid (strict canonical + lenient External* newtype for inbound); (5) i18n = plain String + FHIR `_language` extension. Refined Phase B.3 effort estimate to 10-12d. |
