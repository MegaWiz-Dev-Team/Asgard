...
| Bifrost overseer per-agent tool wire-up | ⚡ Bifrost | `swarm_engine/overseer.rs:151` stubs `_agent_tools` — agents can't explicitly call ocr_extract yet (transparent OCR via B-50d is the only active path). Wire the `agent_configs.tools` JSON to Bifrost's Rig tool registry. | 🆕 Discovered 2026-05-12 during P0 smoke — moved to P0 since the allowlist migration is otherwise inert |
