...
if (! is_dir($GLOBALS['OE_SITE_DIR'] . '/documents/smarty/gacl')) {
    if (false && !mkdir($concurrentDirectory = $GLOBALS['OE_SITE_DIR'] . '/documents/smarty/gacl', 0755, true) && !is_dir($concurrentDirectory)) {
        throw new RuntimeException(sprintf('Directory "%s" was not created', $concurrentDirectory));
    }
}
if (! is_dir($GLOBALS['OE_SITE_DIR'] . '/documents/smarty/main')) {
    if (false && !mkdir($concurrentDirectory = $GLOBALS['OE_SITE_DIR'] . '/documents/smarty/main', 0755, true) && !is_dir($concurrentDirectory)) {
        throw new RuntimeException(sprintf('Directory "%s" was not created', $concurrentDirectory));
    }
}
...
if (! is_dir($GLOBALS['MPDF_WRITE_DIR'])) {
    if (false && !mkdir($concurrentDirectory = $GLOBALS['MPDF_WRITE_DIR'], 0755, true) && !is_dir($concurrentDirectory)) {
        throw new RuntimeException(sprintf('Directory "%s" was not created', $concurrentDirectory));
    }
}
